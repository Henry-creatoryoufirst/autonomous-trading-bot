...
function calculateTradePerformance() {
    ...
    for (const trade of completedTrades) {
        ...
        if (!winsByRegime[regime])
            winsByRegime[regime] = { wins: 0, total: 0 };
        ...
        if (trade.action === "SELL") {
            ...
            if (ret && ret.returnPercent > 0)
                winsByRegime[regime].wins++;
        }
    }
    return {
...
// COST BASIS TRACKING
// ============================================================================
function getOrCreateCostBasis(symbol) {
...
function updateCostBasisAfterBuy(symbol, amountUSD, tokensReceived) {
    const cb = getOrCreateCostBasis(symbol);
    if (cb.totalTokensAcquired === 0)
...
function updateCostBasisAfterSell(symbol, amountUSD, tokensSold) {
    const cb = getOrCreateCostBasis(symbol);
...
function updateUnrealizedPnL(balances) {
    for (const b of balances) {
        if (b.symbol === "USDC" || !state.costBasis[b.symbol])
            continue;
 ...
 */
function checkProfitTaking(balances) {
    if (!CONFIG.trading.profitTaking.enabled)
        return null;
    ...
    for (const b of balances) {
        if (b.symbol === "USDC" || b.usdValue < cfg.minHoldingUSD)
            continue;
        const cb = state.costBasis[b.symbol];
        if (!cb || cb.averageCostBasis <= 0)
            continue;
            ...
            if (gainPercent >= tier.gainPercent) {
                ...
                if (lastTrigger) {
                    ...
                    if (hoursSince < cfg.cooldownHours)
                        continue; // This tier is on cooldown
                }
...
function checkStopLoss(balances) {
    if (!CONFIG.trading.stopLoss.enabled)
        return null;
...
async function fetchDefiLlamaData() {
    try {
        ...
        const [chainRes, protocolsRes, dexVolumeRes] = await Promise.allSettled([
            axios_1.default.get("https://api.llama.fi/v2/historicalChainTvl/Base", { timeout: 10000 }),
            axios_1.default.get("https://api.llama.fi/v2/protocols", { timeout: 15000 }),
            axios_1.default.get("https://api.llama.fi/overview/dexs/base?excludeTotalDataChart=true&excludeTotalDataChartBreakdown=true&dataType=dailyVolume", { timeout: 10000 }),
        ]);
        let baseTVL = 0;
...
let macroCache = { data: null, lastFetch: 0 };
...
async function fetchNewsSentiment() {
    ...
    try {
        ...
        const [bullishRes, bearishRes, risingRes] = await Promise.allSettled([
            axios_1.default.get(`${baseUrl}&filter=bullish`, { timeout: 10000 }),
            axios_1.default.get(`${baseUrl}&filter=bearish`, { timeout: 10000 }),
            axios_1.default.get(`${baseUrl}&filter=rising`, { timeout: 10000 }),
        ]);
        let bullishCount = 0;
        let bearishCount = 0;
        let totalCount = 0;
                ...
                if (item.currencies) {
                    for (const c of item.currencies) {
                        ...
                        if (sym && trackedSymbols.has(sym)) {
                            if (!tokenMentions[sym])
                                tokenMentions[sym] = { bullish: 0, bearish: 0, neutral: 0 };
...
async function fetchMacroData() {
    ...
    try {
        ...
        const [dffRes, dgs10Res, t10y2yRes, cpiRes, m2Res, dollarRes] = await Promise.allSettled([
            axios_1.default.get(`${fredBase}?series_id=DFF${baseParams}`, fredOpts),
            axios_1.default.get(`${fredBase}?series_id=DGS10${baseParams}`, fredOpts),
            axios_1.default.get(`${fredBase}?series_id=T10Y2Y${baseParams}`, fredOpts),
            axios_1.default.get(`${fredBase}?series_id=CPIAUCSL${baseParams}&limit=13`, fredOpts), // 13 months for YoY
            axios_1.default.get(`${fredBase}?series_id=M2SL${baseParams}&limit=13`, fredOpts), // 13 months for YoY
            axios_1.default.get(`${fredBase}?series_id=DTWEXBGS${baseParams}`, fredOpts),
        ]);
        ...
        const parseYoY = (res) => {
            if (res.status !== "fulfilled")
                return null;
            const obs = res.value?.data?.observations?.filter((o) => o.value && o.value !== ".");
            if (!obs || obs.length === 0)
                return null;
        ...
        let rateDirection = "PAUSED";
        if (dffRes.status === "fulfilled") {
            const obs = dffRes.value?.data?.observations?.filter((o) => o.value && o.value !== ".");
            if (obs && obs.length >= 2) {
        ...
        if (cpi)
            console.log(`     CPI: ${cpi.value.toFixed(1)} (${cpi.yoyChange !== null ? `${cpi.yoyChange.toFixed(1)}% YoY` : "N/A"}) | M2: ${m2MoneySupply?.yoyChange !== null ? `${m2MoneySupply?.yoyChange?.toFixed(1)}% YoY` : "N/A"} | Dollar: ${dollarIndex?.value?.toFixed(1) ?? "N/A"}`);
 ...
 */
function determineMarketRegime(fearGreed, indicators, derivatives) {
    ...
    for (const ind of Object.values(indicators)) {
        totalSignals++;
        if (ind.trendDirection === "STRONG_UP" || ind.trendDirection === "UP")
            upSignals++;
        if (ind.trendDirection === "STRONG_DOWN" || ind.trendDirection === "DOWN")
            downSignals++;
    }
    ...
    if (avgBandwidth > 15)
        return "VOLATILE";
    if (upRatio > 0.6 && fearGreed > 40)
        return "TRENDING_UP";
    if (downRatio > 0.6 && fearGreed < 40)
        return "TRENDING_DOWN";
    if (upRatio < 0.4 && downRatio < 0.4)
        return "RANGING";
    return "UNKNOWN";
}
/**
 ...
 */
function formatIntelligenceForPrompt(defi, derivatives, regime, news, macro) {
    const lines = [];
    if (defi) {
        ...
        if (defi.baseTVLChange24h > 3)
            lines.push(`🟢 TVL SIGNAL: Capital flowing INTO Base (+${defi.baseTVLChange24h.toFixed(1)}%) — bullish for Base tokens`);
        else if (defi.baseTVLChange24h < -3)
            lines.push(`🔴 TVL SIGNAL: Capital flowing OUT of Base (${defi.baseTVLChange24h.toFixed(1)}%) — bearish for Base tokens`);
        lines.push("");
    }
    if (derivatives) {
    ...
    if (news) {
        ...
        if (news.sentimentScore > 40)
            lines.push(`🟢 NEWS SIGNAL: Strong bullish sentiment — market optimism, watch for FOMO tops`);
        else if (news.sentimentScore < -40)
            lines.push(`🔴 NEWS SIGNAL: Strong bearish sentiment — market fear, contrarian buying opportunity?`);
        else if (news.overallSentiment === "MIXED")
            lines.push(`⚠️ NEWS SIGNAL: Mixed sentiment — conflicting narratives, use other signals for direction`);
        lines.push("");
    }
    if (macro) {
        lines.push(`═══ MACRO INTELLIGENCE (Federal Reserve / FRED) ═══`);
        if (macro.fedFundsRate)
            lines.push(`Fed Funds Rate: ${macro.fedFundsRate.value.toFixed(2)}% (${macro.rateDirection})`);
        if (macro.treasury10Y)
            lines.push(`10-Year Treasury Yield: ${macro.treasury10Y.value.toFixed(2)}%`);
        if (macro.yieldCurve)
            lines.push(`Yield Curve (10Y-2Y): ${macro.yieldCurve.value >= 0 ? "+" : ""}${macro.yieldCurve.value.toFixed(2)}% ${macro.yieldCurve.value < 0 ? "⚠️ INVERTED" : ""}`);
        if (macro.cpi)
            lines.push(`CPI: ${macro.cpi.value.toFixed(1)} ${macro.cpi.yoyChange !== null ? `(${macro.cpi.yoyChange >= 0 ? "+" : ""}${macro.cpi.yoyChange.toFixed(1)}% YoY)` : ""}`);
        if (macro.m2MoneySupply)
            lines.push(`M2 Money Supply: ${macro.m2MoneySupply.yoyChange !== null ? `${macro.m2MoneySupply.yoyChange >= 0 ? "+" : ""}${macro.m2MoneySupply.yoyChange.toFixed(1)}% YoY` : "N/A"} ${(macro.m2MoneySupply.yoyChange ?? 0) > 5 ? "🟢 LIQUIDITY EXPANDING" : (macro.m2MoneySupply.yoyChange ?? 0) < 0 ? "🔴 LIQUIDITY CONTRACTING" : ""}`);
        if (macro.dollarIndex)
            lines.push(`US Dollar Index: ${macro.dollarIndex.value.toFixed(1)} ${macro.dollarIndex.value > 110 ? "🔴 STRONG (headwind)" : macro.dollarIndex.value < 100 ? "🟢 WEAK (tailwind)" : ""}`);
        lines.push(`Macro Signal: ${macro.macroSignal}`);
        // Macro signal interpretation
        if (macro.macroSignal === "RISK_ON")
            lines.push(`🟢 MACRO SIGNAL: Conditions favor risk assets — looser policy, expanding liquidity, or weakening dollar`);
        else if (macro.macroSignal === "RISK_OFF")
    ...
    switch (regime) {
        case "TRENDING_UP":
            lines.push(`→ Favor buying dips, ride momentum, widen stops`);
            break;
        case "TRENDING_DOWN":
            lines.push(`→ Favor selling rallies, tighten stops, preserve capital`);
            break;
        case "RANGING":
            lines.push(`→ Mean-revert: buy oversold, sell overbought, smaller positions`);
            break;
        case "VOLATILE":
            lines.push(`→ Reduce position sizes, widen stops, wait for clarity`);
            break;
        default:
            lines.push(`→ Mixed signals — use standard rules, stay disciplined`);
            break;
    }
    return lines.join("\n");
}
async function getMarketData() {
    try {
        ...
        const coingeckoIds = [...new Set(Object.values(TOKEN_REGISTRY).map(t => t.coingeckoId).filter(Boolean))].join(",");
        ...
        const intelligencePromise = (async () => {
            const fng = await Promise.allSettled([axios_1.default.get("https://api.alternative.me/fng/", { timeout: 10000 })]).then(r => r[0]);
        ...
        let marketResult = { status: "rejected", reason: new Error("No attempt") };
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                const res = await axios_1.default.get(coingeckoUrl, { timeout: 15000 });
                ...
                if (attempt === 3)
                    marketResult = { status: "rejected", reason: err };
            }
        }
        ...
        if (marketResult.status === "fulfilled") {
            tokens = marketResult.value.data.map((coin) => {
                const registryEntry = Object.entries(TOKEN_REGISTRY).find(([_, t]) => t.coingeckoId === coin.id);
                const symbol = registryEntry ? registryEntry[0] : coin.symbol.toUpperCase();
                const sector = registryEntry ? registryEntry[1].sector : "UNKNOWN";
                return {
            ...
            const reason = marketResult.reason;
 ...
 */
async function fetchPriceHistory(coingeckoId) {
    try {
        const response = await axios_1.default.get(`https://api.coingecko.com/api/v3/coins/${coingeckoId}/market_chart?vs_currency=usd&days=30`, { timeout: 15000 });
        const prices = response.data.prices.map((p) => p[1]);
        const volumes = response.data.total_volumes.map((v) => v[1]);
        const timestamps = response.data.prices.map((p) => p[0]);
 ...
 */
async function getCachedPriceHistory(coingeckoId) {
...
function calculateMACD(prices) {
    if (prices.length < 35)
        return null; // Need at least 26 + 9 periods
...
function calculateBollingerBands(prices, period = 20, stdDevMultiplier = 2) {
    ...
    if (percentB > 1)
        ...
        signal = "SQUEEZE"; // Tight bands = incoming move
...
function determineTrend(prices, sma20, sma50) {
    ...
    if (weeklyChange > 10 && dailyChange > 3 && aboveSMA20 !== false)
        return "STRONG_UP";
    if (weeklyChange > 3 && dailyChange > 0 && aboveSMA20 !== false)
        return "UP";
    if (weeklyChange < -10 && dailyChange < -3 && aboveSMA20 !== true)
        return "STRONG_DOWN";
    if (weeklyChange < -3 && dailyChange < 0 && aboveSMA20 !== true)
        return "DOWN";
    return "SIDEWAYS";
}
/**
...
function calculateConfluence(rsi, macd, bb, trend, priceChange24h, priceChange7d) {
    ...
    if (bb) {
        ...
        if (bb.signal === "OVERSOLD")
            ...
            score += 5; // Squeeze slightly bullish (potential breakout)
        // %B nuance
        if (bb.percentB > 0.8 && bb.percentB <= 1)
            score -= 5;
        else if (bb.percentB < 0.2 && bb.percentB >= 0)
            score += 5;
    }
    // Trend (weight: 15)
    signals++;
 ...
 */
async function computeIndicators(coingeckoId, currentPrice, priceChange24h, priceChange7d, volume24h) {
 ...
 */
async function getTokenIndicators(tokens) {
    const indicators = {};
    ...
    for (let i = 0; i < uniqueTokens.length; i += 3) {
        ...
        const results = await Promise.allSettled(batch.map(t => computeIndicators(t.coingeckoId, t.price, t.change24h, t.change7d, t.volume)));
 ...
 */
function formatIndicatorsForPrompt(indicators, tokens) {
    const lines = [];
    for (const token of tokens) {
...
async function rpcCall(method, params) {
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            const response = await axios_1.default.post(BASE_RPC_URL, {
...
// SECTOR ANALYSIS
// ============================================================================
function calculateSectorAllocations(balances, totalValue) {
    const allocations = [];
    for (const [sectorKey, sectorInfo] of Object.entries(SECTORS)) {
        const sectorTokens = balances.filter(b => sectorInfo.tokens.includes(b.symbol) && b.usdValue > 0);
...
async function makeTradeDecision(balances, marketData, totalPortfolioValue, sectorAllocations) {
    const usdcBalance = balances.find(b => b.symbol === "USDC");
    const availableUSDC = usdcBalance?.balance || 0;
    const holdingsBySector = {};
    for (const allocation of sectorAllocations) {
        holdingsBySector[allocation.name] = allocation.tokens.map(t => `${t.symbol}: $${t.usdValue.toFixed(2)} (${t.percent.toFixed(1)}%)`);
    }
    ...
    for (const token of marketData.tokens) {
        ...
        if (!marketBySector[sector])
            marketBySector[sector] = [];
        marketBySector[sector].push(`${token.symbol}: $${token.price < 1 ? token.price.toFixed(6) : token.price.toFixed(2)} (24h: ${token.priceChange24h >= 0 ? "+" : ""}${token.priceChange24h.toFixed(1)}%)`);
    }
    ...
    const tradeHistorySummary = recentTrades.length > 0
        ? recentTrades.map(t => `  ${t.timestamp.slice(5, 16)} ${t.action} ${t.fromToken}→${t.toToken} $${t.amountUSD.toFixed(2)} ${t.success ? "✅" : "❌"} regime=${t.signalContext?.marketRegime || "?"} ${t.reasoning?.substring(0, 60) || ""}`).join("\n")
        : "  No trades yet";
...
═══ SECTOR ALLOCATIONS ═══
${sectorAllocations.map(s => `${s.drift > 5 ? "⚠️OVER" : s.drift < -5 ? "⚠️UNDER" : "✅"} ${s.name}: ${s.currentPercent.toFixed(1)}% (target: ${s.targetPercent}%) drift: ${s.drift >= 0 ? "+" : ""}${s.drift.toFixed(1)}%`).join("\n")}

═══ HOLDINGS ═══
${Object.entries(holdingsBySector).map(([sector, holdings]) => `${sector}: ${holdings.length > 0 ? holdings.join(" | ") : "Empty"}`).join("\n")}
...
═══ TOKEN PRICES ═══
${Object.entries(marketBySector).map(([sector, tokens]) => `${sector}: ${tokens.slice(0, 5).join(" | ")}`).join("\n")}
...
async function executeTrade(decision, marketData) {
    ...
    try {
        ...
        const fromTokenAddress = getTokenAddress(decision.fromToken);
        const toTokenAddress = getTokenAddress(decision.toToken);
        ...
        if (decision.fromToken === "USDC") {
            ...
            fromAmount = (0, viem_1.parseUnits)(decision.amountUSD.toFixed(6), 6);
        }
        else {
            ...
            fromAmount = (0, viem_1.parseUnits)(tokenAmount.toFixed(Math.min(fromDecimals, 8)), fromDecimals);
        }
        console.log(`\n  🔄 EXECUTING TRADE via CDP SDK:`);
        console.log(`     ${decision.fromToken} (${fromTokenAddress})`);
        console.log(`     → ${decision.toToken} (${toTokenAddress})`);
        console.log(`     Amount: ${(0, viem_1.formatUnits)(fromAmount, fromDecimals)} ${decision.fromToken} (~$${decision.amountUSD.toFixed(2)})`);
        ...
        if (error.response?.data) {
            try {
                ...
                console.error(`     API Response: [non-serializable]`);
            }
        }
        if (error.stack)
    ...
    try {
        ...
        if (Object.keys(marketData.indicators).length > 0) {
            ...
            for (const [symbol, ind] of Object.entries(marketData.indicators)) {
                ...
                if (ind.confluenceScore >= 30)
                    buySignals.push(`${symbol}(+${ind.confluenceScore})`);
                if (ind.confluenceScore <= -30)
                    sellSignals.push(`${symbol}(${ind.confluenceScore})`);
            }
            if (buySignals.length > 0)
                console.log(`   🟢 Buy signals: ${buySignals.join(", ")}`);
            if (sellSignals.length > 0)
                console.log(`   🔴 Sell signals: ${sellSignals.join(", ")}`);
        }
        console.log(`\n📊 Sector Allocations:`);

...
## Run #38 — 2026-06-28T09:xx UTC

### Jobs This Run
| Job | Status |
|-----|--------|
| 🏥 Medic | PATTERN D — `/api/errors`, `/api/balances` → 403 (egress proxy policy, same since Run #1) |
| 🔍 Scout | BLOCKED — `api.geckoterminal.com` → 403; CLAUDE.md Rule 1 unchanged; 34 days since last scout (2026-05-25) |
| 📊 Auditor | BLOCKED — all portfolio endpoints 403; cannot compute win_rate, drawdown, or streak |

### New Development: Base Beryl B20 Tokens Now 3 Days Old
Base Beryl upgrade activated June 25. B20 native tokens have been live for 3 days — pools created on June 25 NOW pass the scout's "pool age > 3 days" quality filter. This is the first run where B20 tokens could qualify. GeckoTerminal is still blocked from this environment, but Henry can manually check `https://www.geckoterminal.com/explore/new-crypto-pools/base` for B20-native pools with:
- Liquidity > $100k
- 24h volume > $50k
- Pool age ≥ 3 days (i.e., created on or before June 25)

Any qualifying candidates: write a `COHORT_PROPOSAL_2026-06-28.md` per CLAUDE.md Rule 1 (requires human PR, not auto-add).

### Bot Status
- Bot has been in full USDC for ~31 days (since May 28 liquidation via `/api/admin/liquidate-all`)
- Last production code change: 2026-05-28 (31 days ago — commit `f29798d`)
- Verify Railway deployment status and whether bot has re-entered positions

### Persistent Ask (Run #38 of #38)
Add to Claude Code egress allowlist at https://code.claude.com/docs/en/claude-code-on-the-web:
...
| #35–37 | 2026-06-22 to 2026-06-26 UTC | All jobs PATTERN D — API unreachable. Option B window closed ~2026-06-15. Base Beryl upgrade activated June 25 (B20 native token standard). Bot confirmed in full USDC for 27–29 days. CLAUDE.md Rule 1 still in effect (not lifted by Henry). |
| #38 | 2026-06-28T09:xx UTC | All jobs PATTERN D — API unreachable. B20 tokens now 3 days old (cross scout pool-age threshold). Bot in full USDC ~31 days. Scout 34 days overdue. |
